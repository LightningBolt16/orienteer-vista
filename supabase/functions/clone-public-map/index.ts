...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Check if this map has a source user_maps record
    if (!routeMap.source_map_id) {
      return new Response(
        JSON.stringify({ error: 'This map has no source files to clone' }),
        { status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
      );
    }

    // Get the source user_maps record
    ...
    // Create a new user_maps record for this user, referencing the same R2 keys
    // The user will upload a modified B&W file after editing
